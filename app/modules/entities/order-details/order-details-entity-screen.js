...
import { orderDetailEntityDetailScreen, orderDetailEntityEditScreen } from '../../../navigation/layouts'
...
class OrderDetailEntityScreen extends React.PureComponent {
  ...
  navigationButtonPressed({ buttonId }) {
    orderDetailEntityEditScreen({ entityId: null })
  }
  /* ***********************************************************
  * STEP 2
  ...
  *************************************************************/
  renderRow({ item }) {
    Alert.alert('arun');
    return (
      <TouchableOpacity onPress={orderDetailEntityDetailScreen.bind(this, { entityId: item.id })}>
  ...
  renderEmpty = () => <AlertMessage title="No OrderDetails Found" show={!this.props.fetching} />
  ...
  fetchOrderDetails = () => {
    this.props.getAllOrderDetails({ page: this.state.page, sort: this.state.sort, size: this.state.size })
  }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (nextProps.orderDetails) {
      return {
        done: nextProps.orderDetails.length < prevState.size,
        dataObjects: [...prevState.dataObjects, ...nextProps.orderDetails],
      }
    }
    return null
  }
...
const mapDispatchToProps = dispatch => {
  return {
    performSearch: query => dispatch(OrderDetailActions.orderDetailSearchRequest(query)),
